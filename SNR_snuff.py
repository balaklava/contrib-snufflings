...
def readMarker(self,phase_markers,event_marker):
    '''
        Creates the dictionary of phases for one event. 
	The key is nslc - network, station, location, channel.
    '''
...
def ProcessTrace(self, evm, p, s, tr_list, selected_sta1, selected_sta2,list_of_channels):
    ...
    for i in range(len(tr_list)): 
        if tr_list[i].tmin < evm.tmin and evm.tmin < tr_list[i].tmax: 
			...
			"2. There is a gap in the trace data for the event " 
			+ util.time_to_str(evm.tmin, format='%Y-%m-%d %H:%M:%S.3FRAC') 
			+ " (remove this event)");
    ...
    for i in range(len(p.choped_trace)):
        ...
        taper_p = trace.CosTaper(p.choped_trace[i].tmin, 
				p.choped_trace[i].tmin + deltat_taper, 
				p.choped_trace[i].tmax - deltat_taper, 
				p.choped_trace[i].tmax)
        taper_s = trace.CosTaper(s.choped_trace[i].tmin, 
				s.choped_trace[i].tmin + deltat_taper, 
				s.choped_trace[i].tmax - deltat_taper, 
				s.choped_trace[i].tmax)

        p.choped_trace[i].taper(taper_p, inplace=True)
        s.choped_trace[i].taper(taper_s, inplace=True)

    #FFT and the amplitude spectra (
    #	can be done together with tapering, if to use the parameter tfade)
    bandwidth = 40
    for i in range(len(p.choped_trace)):
    ...
    if len(list_of_channels) == 1:
        ...
        ffty_sta1 = stats.gmean((s.ffty[ista1[0]]/p.ffty[ista1[0]], 
				s.ffty[ista1[1]]/p.ffty[ista1[1]], 
				s.ffty[ista1[2]]/p.ffty[ista1[2]]))
        ffty_sta2 = stats.gmean((s.ffty[ista2[0]]/p.ffty[ista2[0]], 
				s.ffty[ista2[1]]/p.ffty[ista2[1]], 
				s.ffty[ista2[2]]/p.ffty[ista2[2]]))  
